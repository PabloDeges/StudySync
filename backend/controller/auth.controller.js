...
const registierung = async (req, res, next) => {
    try {
        ...
        if((await pool.query(`select benutzername, pwhash from studysync.benutzer where benutzername = '${username}'`)).rowCount >= 1){
            res.status(500).json({ message: "Nutzer schon vorhanden" });
        } else {
            const hashedPassword = await bcrypt.hash(password, 10);
            let result = await pool.query(`INSERT INTO studysync.benutzer(benutzername, pwhash, studiengangid) VALUES ('${username}','${hashedPassword}',3)`);
...
const login = async (req, res) => {
    try {
        ...
        if ((await pool.query(`select benutzername, pwhash from studysync.benutzer where benutzername = '${username}'`)).rowCount >= 1) {
            const userpw = (await pool.query(`select benutzername, pwhash from studysync.benutzer where benutzername = '${username}'`)).rows[0].pwhash
